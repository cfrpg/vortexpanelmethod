...
"""A set of functions to compute the coordinates of NACA airfoils.

In this version, only the NACA 4-digit airfoil series has been implemented.
"""
...
def NACA4(number, N, half_cosine_spacing=True, closed_trailing_edge=True, save_to_file=False):
    ...
    if half_cosine_spacing:
        x = (1 - np.cos(np.linspace(0, pi, N))) / 2
    else:
        x = np.linspace(0, 1, N)
